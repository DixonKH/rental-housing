...
const TuiEditor = () => {
	...
	const uploadImage = async (image: any) => {
		try {
			...
			const response = await axios.post(`${process.env.NEXT_PUBLIC_AI_GRAPHQL_URL}`, formData, {
				headers: {
	...
	const handleRegisterButton = async () => {
		try {
			...
			await createBoardArticle({ variables: { input: { ...memoizedValues, articleCategory } } });
			await sweetTopSuccessAlert('Article is created successfully', 700);
			await router.push({
				pathname: '/mypage',
				query: { category: 'myArticle' },
			});
		} catch (err: any) {
			console.log(err);
